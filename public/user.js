...
function getPos(p){
return (p.position || p.pos || p.role || "").toUpperCase();
}
...
if(existingPrediction) autofillPrediction();
...
if(id==="gk"){
pool = squad.filter(p => getPos(p) === "GK");
}else{
pool = squad.filter(p => getPos(p) !== "GK");
}
...
function pickPlayer(callback,players){
...
players: lineup.map(p=>p._id),

subs: subs.filter(Boolean).map(p=>p._id),
...
if(lineup.filter(Boolean).length !== 11){
...
const res=await fetch(`/matches/${currentMatch._id}/predict`,{
...
// team: currentMatch.homeTeam._id || currentMatch.homeTeam,

// players: lineup.map(p=>p._id),

// subs: subs.filter(p=>p).map(p=>p._id),
