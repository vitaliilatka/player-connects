...
router.post("/:matchId/predict-lineup", authMiddleware(), async (req, res) => {
  try {
    ...
    console.log("REAL TEAM NAME:", teamName);


    // определяем реальную команду пользователя
    const teamSide = team; // "home" или "away"

    let realTeamName = null;

    if (teamSide === "home") {
      realTeamName = match.homeTeam;
    } else {
      realTeamName = match.awayTeam;
    }

    console.log("TEAM SIDE:", teamSide);
    console.log("REAL TEAM NAME:", realTeamName);

    // 🔥 БЕРЁМ ИГРОКОВ ИЗ PLAYERS (НЕ TeamSquad)
