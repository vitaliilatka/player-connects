...
import { rollbackMatchStats } from "../services/rollbackMatchStats.js";
...
router.post("/:id/full", authMiddleware("admin"), async (req, res) => {
  ...
  try {
    ...
    if (match.statsProcessed) {
      console.log("Rolling back old stats...");
      await rollbackMatchStats(match);
    }
    ...
    match.events.missedPenalties.home =
      (missedPenalties.home || []).map(p => ({
        minute: p.minute,
        takenBy: new mongoose.Types.ObjectId(p.player),
        earnedBy: null
      }));

    match.events.missedPenalties.away =
      (missedPenalties.away || []).map(p => ({
        minute: p.minute,
        takenBy: new mongoose.Types.ObjectId(p.player),
        earnedBy: null
      }));
